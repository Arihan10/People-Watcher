Shader "Stylized/Water"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            TEXTURE2D(_SparklesNormalMap);
            SAMPLER(sampler_SparklesNormalMap);
            ...
            half4 frag (Varyings i) : SV_Target
            {
                ...
                float3x3 tangentToWorld = GetWaveTBN(i.positionWS.xz, 0.01);

                // Sample the wave normal map and calculate the world-space normal for this fragment.
                float3 normalTS = MotionFourWayChaos(TEXTURE2D_ARGS(_WaveNormalMap, sampler_WaveNormalMap), i.positionWS.xz/_WaveNormalScale, _WaveNormalSpeed, true);
                ...
                // Sample the opaque texture (URP's replacement for GrabPass) and depth textures
                float3 fragColor = SampleSceneColor(screenCoord).rgb;
                float fragDepth = SampleSceneDepth(screenCoord);
                ...
                float sceneEyeDepth = LinearEyeDepth(fragDepth, _ZBufferParams);
                float surfaceEyeDepth = LinearEyeDepth(i.positionCS.z / i.positionCS.w, _ZBufferParams);
                float opticalDepth = abs(sceneEyeDepth - surfaceEyeDepth);
                float transmittance = exp(-_DepthDensity * opticalDepth);

                // Also calculate how far away the fragment is from the camera.
                float distanceMask = exp(-_DistanceDensity * length(i.positionWS - _WorldSpaceCameraPos));
                ...
                // If shadows are enabled, sample the shadow map using URP's system.
                #ifdef SHADOWS
                    shadowMask = GetURPMainLightShadow(i.positionWS, _MaxShadowDistance);
                #endif
                ...
                float3 reflectedColor = SAMPLE_TEXTURECUBE(unity_SpecCube0, samplerunity_SpecCube0, reflectDir).rgb;
                ...
                float3 sssColor = _SSSColor * GetWaveHeight(i.positionWS.xz);

                // Only draw it on wavetips that the sun is shining through (so it appears backlit).
                sssColor *= saturate(dot(viewDirWS, lightDir));
                ...
                float2 foamUV = (i.positionWS.xz / _FoamScale) + (_FoamNoiseScale * normalTS.xz);

                // Sample the foam texture and modulate the result by the distance mask and shadow mask.
                float3 foamColor = MotionFourWayChaos(TEXTURE2D_ARGS(_FoamTexture, sampler_FoamTexture), foamUV, _FoamSpeed, false);
                ...
                float sunSpecularMask = saturate(dot(viewR, lightDir));
                ...
                float3 sunSpecularColor = lerp(float3(0,0,0), _SunSpecularColor, sunSpecularMask);
                ...
                float3 sparkly1 = MotionFourWaySparkle(TEXTURE2D_ARGS(_SparklesNormalMap, sampler_SparklesNormalMap), i.positionWS.xz / _SparkleScale, float4(1,2,3,4), _SparkleSpeed);
                float3 sparkly2 = MotionFourWaySparkle(TEXTURE2D_ARGS(_SparklesNormalMap, sampler_SparklesNormalMap), i.positionWS.xz / _SparkleScale, float4(1,0.5,2.5,2), _SparkleSpeed);
                ...
                float3 sparkleColor = lerp(float3(0,0,0), _SparkleColor, sparkleMask);
                ...
                float3 edgeFoamColor = lerp(float3(0,0,0), _EdgeFoamColor, edgeFoamMask);
                edgeFoamColor = edgeFoamColor * lerp(_ShadowColor, float3(1,1,1), shadowMask);
